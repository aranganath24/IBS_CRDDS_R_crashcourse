...
As well as cross-tabs, which show the frequency distribution of two or more categorical variables; one useful function for generating crosstabs in R is the `table()` function, that takes dataset columns as arguments.
...
trade_cgexp_cc<-cor.test(pt_copy$trade, pt_copy$cgexp, use="complete.obs")
```
...
Think about some basic operations you could implement to clean or modify the four datasets in `world_bank_list`. Write a function to implement these procedures, then iteratively apply these procedures to the datasets using a `map()` function. When you're done, write out the cleaned datasets to a local directory on your computer.
